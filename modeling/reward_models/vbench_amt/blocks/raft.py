...
# NOTE: upstream 原样 (RAVEN: project/models/reward_models/vbench_amt/blocks/raft.py:4); MFU
...
# NOTE: upstream 原样 (RAVEN: project/models/reward_models/vbench_amt/blocks/raft.py:143);
# CustomFlops comes from the unported project.utils.mfu; the wrapper only needs nn.Module.
#         class CorrelationVolume(torch.nn.Module, CustomFlops):
class CorrelationVolume(torch.nn.Module):
# <<<<<<<<<<<<<< end RAVEN adaptation <<<<<<<<<<<<<<
    def forward(self, fmap1, fmap2):
        ...
        return corr / torch.sqrt(torch.tensor(dim, device=corr.device, dtype=corr.dtype))

    def tflops(self, args, kwargs, output) -> float:
        del output
        fmap1 = args[0] if len(args) > 0 else kwargs["fmap1"]
        batch, dim, ht, wd = fmap1.shape
        num_positions = ht * wd
        return 2 * batch * dim * (num_positions / 1e6) * (num_positions / 1e6)


class BidirCorrBlock:
    def __init__(self, fmap1, fmap2, num_levels=4, radius=4, corr_volume=None):
        self.num_levels = num_levels
        self.radius = radius
        self.corr_pyramid = []
        self.corr_pyramid_T = []

        corr = corr_volume(fmap1, fmap2) if corr_volume is not None else BidirCorrBlock.corr(fmap1, fmap2)
