...
# ===== JARVIS PATCH BEGIN(sglang custom kernels unavailable; upstream eager fallbacks below are used) =====
...
def _scaled_residual_add(residual, x, scale):
    # ===== JARVIS PATCH BEGIN(sglang triton fused scaled-residual-add unavailable) =====
...
class FeedForward(nn.Module):
    ...
    def _forward_impl(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        if self.use_gated:
            if (
                # ===== JARVIS PATCH BEGIN(sglang silu_and_mul_with_activation_rounding unavailable) =====
                False
                and
                # ===== JARVIS PATCH END =====
