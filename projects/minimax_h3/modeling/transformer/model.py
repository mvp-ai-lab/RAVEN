...
# ===== JARVIS PATCH BEGIN(training: sglang ships neither a trainable attention backend nor gradient checkpointing; pull in this package's copies of the repo's varlen FlashAttention wrapper and segment-wise checkpoint helper) =====
from utils.flash_attn import FlashAttention
from ..checkpointing import (
    maybe_checkpoint,
)

# The wrapper holds no parameters or buffers, so a module-level instance keeps
# the DiT's module tree - and therefore its state dict - untouched.
_MINIMAX_H3_FLASH_ATTENTION = FlashAttention()
# ===== JARVIS PATCH END =====

# ===== JARVIS PATCH BEGIN(sglang runtime coupling: sglang is not a dependency of this repo, so its custom kernels, TP/parallel linears, attention-backend selector, layerwise-offload manager, BaseDiT and breakable-CUDA-graph decorator are replaced by TP=1/no-kernel stand-ins; the vendored config moves to this package) =====
...
# Every fused-kernel call site below is an upstream fast path with an eager
# fallback in the same function; this single flag disables all of them.
_SGL_KERNELS_AVAILABLE = False


class envs:  # noqa: N801 - stands in for sglang.multimodal_gen.envs
...
def is_layerwise_offloaded_module(_module) -> bool:
    return False


class BaseDiT(nn.Module):
    """Same __init__ contract as sglang's BaseDiT, without the ABC/TeaCache parts."""

    def __init__(self, config: Any, hf_config: dict[str, Any], **kwargs: Any) -> None:
        super().__init__()
        self.config = config
...
def get_tp_world_size() -> int:
    """TP is not vendored; this build is always TP=1."""
    return 1


def eager_on_graph(_enabled: bool):
    """No-op stand-in for sglang's breakable-CUDA-graph marker."""
...
class _Linear(nn.Module):
    """nn.Linear with sglang's `(output, output_bias)` return contract.

    At TP=1, ColumnParallelLinear / MergedColumnParallelLinear /
    RowParallelLinear all register exactly `weight` [out, in] and, when
    bias=True, `bias` [out] via UnquantizedLinearMethod.create_weights, add the
    bias inside forward (skip_bias_add defaults to False), and perform no
    collectives (gather_output / input_is_parallel are no-ops for world_size 1).
    So this swap leaves the parameter names, shapes, dtypes, and math
    unchanged; only the TP sharding machinery is dropped. MergedColumnParallelLinear
    over output_sizes [a, b, ...] holds one concatenated `weight` of
    out_features=sum(output_sizes), which is what nn.Linear stores too.
    """
        ...
        *,
        ...
        if quant_config is not None:
            raise NotImplementedError(
                "quantized MiniMax H3 linears are not vendored; "
                f"got {quant_config!r} for {prefix!r}"
            )
        out_features = (
            sum(output_size) if isinstance(output_size, list) else int(output_size)
        )
        self.input_size = int(input_size)
        self.output_size = out_features
        self.tp_rank = 0
        # ===== JARVIS PATCH BEGIN(training: sglang's UnquantizedLinearMethod.create_weights builds inference-only weights, so upstream passes requires_grad=False here; that would ship 99.9% of the DiT frozen, and freezing is a config concern in this repo (runtime.requires_grad), so keep the nn.Parameter default of True) =====
        self.weight = nn.Parameter(
            torch.empty(out_features, int(input_size), dtype=params_dtype),
            # requires_grad=False,
        )
        # ===== JARVIS PATCH END =====
        self.weight.weight_loader = _default_weight_loader
        if bias:
            # ===== JARVIS PATCH BEGIN(training: same as the weight above; the bias must default to requires_grad=True) =====
            self.bias = nn.Parameter(
                torch.empty(out_features, dtype=params_dtype),  # requires_grad=False
            )
            # ===== JARVIS PATCH END =====
            self.bias.weight_loader = _default_weight_loader
        else:
...
def _ulysses_ctx() -> tuple[int, int]:
    ...
    """
    # ===== JARVIS PATCH BEGIN(sglang runtime coupling: no distributed init here; this is the upstream uninitialized branch) =====
...
def _modulate_scale_shift(
    ...
    *,
    ...
    if (
        # ===== JARVIS PATCH BEGIN(sglang custom kernel not vendored: disable the fused fast path, keep upstream's eager fallback below) =====
        _SGL_KERNELS_AVAILABLE
        and
        # ===== JARVIS PATCH END =====
        x.is_cuda
...
def _apply_rope_qk(
    ...
    if (
        not q.is_cuda
        # ===== JARVIS PATCH BEGIN(sgl_kernel not vendored: force upstream's explicit torch RoPE path, which keeps RoPE a single identifiable step on Q/K) =====
        or not _SGL_KERNELS_AVAILABLE
        # ===== JARVIS PATCH END =====
    ):
...
def _minimax_h3_attention_core_impl(
    ...
    *,
    ...
    # ===== JARVIS PATCH BEGIN(sglang runtime coupling: the attention-backend selector is not vendored and has no upstream fallback; training runs the repo's varlen FlashAttention wrapper (FA2/FA3 have backward kernels, SDPA fallback off-CUDA), keeping causal=False, the softmax scale, and cu_seqlens semantics) =====
    ...
    # )
    # The wrapper takes per-document lengths and rebuilds cu_seqlens by cumsum,
    # which reproduces this cu_seqlens exactly (it always starts at 0). It also
    # derives max_seqlen itself, and falls back to the varlen SDPA loop that
    # used to be inlined here when neither FA2 nor FA3 is installed. Keep the
    # lengths on the host when cu_seqlens_host is available: the wrapper reads
    # max(q_lens).item(), which would otherwise sync the device once per block.
    del max_seqlen
    if cu_seqlens_host is not None:
        seqlens = torch.tensor(
            [stop - start for start, stop in zip(cu_seqlens_host[:-1], cu_seqlens_host[1:])],
            dtype=torch.int32,
        )
    else:
    ...
    def __init__(
        ...
        *,
        ...
        self._use_fused_qknorm_rope = (
            # ===== JARVIS PATCH BEGIN(sglang custom kernel not vendored: disable fused qknorm+RoPE so q/k norm and RoPE stay separate explicit steps) =====
            _SGL_KERNELS_AVAILABLE
            and
            # ===== JARVIS PATCH END =====
...
class MiniMaxH3TokenRefiner(nn.Module):
    def __init__(
        ...
        *,
        ...
        # ===== JARVIS PATCH BEGIN(training: sglang has no gradient checkpointing; MiniMaxH3DiTModel.set_gradient_checkpointing drives this flag) =====
    ...
    def forward(
        ...
        *,
        ...
        # ===== JARVIS PATCH BEGIN(training: run the refiner stack through the checkpoint helper; every block takes the same kwargs and returns one tensor, so one block per segment) =====
...
class MiniMaxH3DiTModel(BaseDiT, LayerwiseOffloadableModuleMixin):
    ...
    def _validate_sequence_parallel_config(
        *,
        ...
        # ===== JARVIS PATCH BEGIN(sequence parallelism: this check is vacuous in this build - ulysses_size comes from the hard-stubbed _ulysses_ctx() and is always 1. The live head-divisibility constraint is enforced by _ulysses_scatter_heads in model_sp.py, which is where the SP subclasses actually shard the heads) =====
    ...
    def __init__(
        ...
        # ===== JARVIS PATCH BEGIN(training: sglang has no gradient checkpointing; these mirror the wan2_1 model's checkpointing state) =====
        self.gradient_checkpointing = False
        self.block_checkpoint_start_idx = 0
        self.block_checkpoint_step = 1

    def set_gradient_checkpointing(self, enabled=True, gc_start_idx=0, gc_step=1):
        """Signature matches the other families', so common.plugin.gradient_checkpointing.NativeGradientCheckpointing drives it."""
        self.gradient_checkpointing = enabled
        self.block_checkpoint_start_idx = gc_start_idx
        self.block_checkpoint_step = gc_step
        self.token_refiner.gradient_checkpointing = enabled
        # ===== JARVIS PATCH END =====
    ...
    def _validate_taps(
        ...
        if tap_blocks is None:
            return None
        if not isinstance(tap_blocks, (list, tuple)):
            raise TypeError("tap_blocks must be a list or tuple of block counts")
        if not tap_blocks:
            raise ValueError("tap_blocks must contain at least one block count")
        if any(type(block) is not int for block in tap_blocks):
            raise TypeError("tap_blocks must contain only integers")
        if any(block < 1 or block > len(self.blocks) for block in tap_blocks):
            raise ValueError(
                "Each tap_blocks value is a count of leading blocks and must be in "
                f"[1, {len(self.blocks)}], got {tap_blocks}"
            )
        if any(
            left >= right for left, right in zip(tap_blocks, tap_blocks[1:])
        ):
            raise ValueError("tap_blocks must be strictly increasing")
        if tap_outputs is None:
            raise ValueError("tap_outputs must be provided when tap_blocks is set")
        return tuple(tap_blocks)

    def _checkpoint_block_stack(
        self,
        hidden: torch.Tensor,
        *,
        tap_blocks: tuple[int, ...] | None,
        tap_outputs: list[torch.Tensor] | None,
        **block_kwargs: Any,
    ) -> torch.Tensor:
        if tap_blocks is None:
        ...
        assert tap_outputs is not None
        start_block = 0
        tap_index = 0
        for end_block in (*tap_blocks, len(self.blocks)):
            if end_block == start_block:
                continue
            blocks = self.blocks[start_block:end_block]
            segment_gc_start_idx = min(
                max(self.block_checkpoint_start_idx - start_block, 0),
                len(blocks),
            )
            hidden = maybe_checkpoint(
                ...
                enabled=self.gradient_checkpointing,
                gc_step=self.block_checkpoint_step,
                gc_start_idx=segment_gc_start_idx,
                use_reentrant=False,
                **block_kwargs,
            )
            if tap_index < len(tap_blocks) and end_block == tap_blocks[tap_index]:
                tap_outputs.append(hidden)
                tap_index += 1
            start_block = end_block
        return hidden

    def _resolve_attention_backend_once(self) -> None:
        # ===== JARVIS PATCH BEGIN(sglang runtime coupling: get_attn_backend is not vendored; attention runs the copied varlen FlashAttention wrapper instead) =====
        return
        # ===== JARVIS PATCH END =====
    ...
    def refine_prompt_embeds(
        ...
        *,
        ...
        # ===== JARVIS PATCH BEGIN(batch packing: refine every live text document described by the caller instead of fabricating one document plus an empty tail) =====
        """Project and refine request-static text conditioning once.

        ``refiner_cu_seqlens`` has B+1 entries for exactly the B live text
        documents: it starts at zero and ends at the total live text rows.
        Padding lies beyond that final offset and is not described.
        """
        lens = refiner_cu_seqlens.tolist()
        text_len = lens[-1]
        ...
        return self.token_refiner(
            ...
            cu_seqlens_host=tuple(lens),
            max_seqlen=max(b - a for a, b in zip(lens[:-1], lens[1:])),
        )
        # ===== JARVIS PATCH END =====

    # ===== JARVIS PATCH BEGIN(sequence parallelism: build_rope_cache read _ulysses_ctx() directly, which is hard-stubbed to (1, 0), so it never sliced by rank. Route it through this seam instead; the SP subclasses override the seam with this repo's unified_parallel context, matching what their forwards already do) =====
    def _sequence_parallel_ctx(self) -> tuple[int, int]:
        """(world_size, rank) of the sequence-parallel group this model runs in."""
        return _ulysses_ctx()
    ...
    def build_rope_cache(
        ...
        *,
        ...
        # ===== JARVIS PATCH BEGIN(sequence parallelism: was _ulysses_ctx(); see the seam above) =====
        sp_ws, sp_rank = self._sequence_parallel_ctx()
        # ===== JARVIS PATCH END =====
        if seq_len % sp_ws:
            raise ValueError(
    ...
    def _embed(
        ...
        *,
        ...
        refiner_cu_seqlens: torch.Tensor,
        # ===== JARVIS PATCH BEGIN(note only: refiner_max_seqlen is never read by this body - refine_prompt_embeds derives the maximum document length from the live text cu_seqlens. Kept in the signature so all four call sites stay uniform with upstream) =====
        ...
        """
        # ===== JARVIS PATCH BEGIN(batch packing: the final refiner offset is the total live text length; trusted local layouts can only express one leading contiguous text source slice) =====
        if local_embedding_layout is not None and refiner_cu_seqlens.numel() > 2:
            raise ValueError(
                "local_embedding_layout requires one contiguous text source slice "
                "to own local rows [0, text_rows), so it cannot represent B>1 "
                "packed text documents."
            )
        ...
        # ===== JARVIS PATCH END =====
        # ===== JARVIS PATCH BEGIN(zelda dual-drop: a pack whose every text-side condition is dropped carries zero live text rows; there is nothing to refine, and the varlen refiner cannot express an empty pack, so it is skipped rather than called) =====
        if text_len < 0 or text_len > int(text_embeddings_selected.shape[0]):
            raise ValueError(
                "refiner cu_seqlens live text length must be in "
                f"[0, {int(text_embeddings_selected.shape[0])}], got {text_len}"
            )
        ...
        if refined_prompt_embeds_length is not None:
            ...
            text_embed = torch.zeros(
                (0, self.hidden_size), device=device, dtype=_BF16_DTYPE
            )
        else:
        ...
        # ===== JARVIS PATCH BEGIN(sequence parallelism: this write was guarded by text_row_ids.numel(). The packed layout puts the text rows first, so under SP only the group's rank 0 owns any, and on every other rank the guard skipped the write and severed condition_proj + token_refiner from the graph -- their gradients came back None there and were fine elsewhere. FSDP2 builds its reduce-scatter input from the parameters that HAVE gradients (_fsdp_param_group.py post_backward), so a rank-dependent gradient set makes the collective rank-dependent too: the reduce-scatters cross-match and every rank spins in an NCCL kernel forever. An index_add_ with an empty index is a numerical no-op but keeps the autograd edge, so the refiner gets a zero gradient on the ranks that hold no text) =====
        else:
            write_rows(
                0,
                text_row_ids,
        ...
        # ===== JARVIS PATCH BEGIN(sequence parallelism: both writes were guarded by their row count, and both are unconditional now for the same reason as the text write above -- a row window holding no video rows, or no audio rows, would skip the projection and leave video_patch_proj / audio_patch_proj without a gradient on that rank alone, which makes FSDP2's reduce-scatter input rank-dependent and deadlocks it. Unlike the text case this is prevention rather than a fix for an observed hang: the chunk layout interleaves audio and video densely enough that every window held both at the up_sizes measured, and the grad-None repro found only the text parameters missing) =====
        x_rows = (
    ...
    def forward(self, **kwargs: Any) -> tuple[torch.Tensor, torch.Tensor]:
        # ===== JARVIS PATCH BEGIN(batch packing contract: document the live-text cu_seqlens and row ordering consumed by the refiner) =====
        ...
        those per-sample text rows in document order.
        ...
        if sp_ws > 1:
            ...
            # ===== JARVIS PATCH BEGIN(sequence parallelism: this check is vacuous in this build - sp_ws comes from the hard-stubbed _ulysses_ctx() and is always 1, so this whole branch is dead. The live head-divisibility constraint is enforced by _ulysses_scatter_heads in model_sp.py, which is where the SP subclasses actually shard the heads) =====
        ...
        # ===== JARVIS PATCH BEGIN(training: sglang has no gradient checkpointing; route the block stack through the checkpoint helper. The upstream per-block loop stays for the batched-AdaLN path, whose adaln_params differ per block and so cannot be shared across a checkpointed segment) =====
        if block_adaln_params is None:
            ...
            for index, block in enumerate(self.blocks):
                hidden = block(
                    ...
                    max_seqlen=max_seqlen,
                    ulysses_active=sp_ws > 1,
                    adaln_params=block_adaln_params[index],
                )
                if (
                    tap_blocks is not None
                    and tap_index < len(tap_blocks)
                    and index + 1 == tap_blocks[tap_index]
                ):
                    assert tap_outputs is not None
                    tap_outputs.append(hidden)
                    tap_index += 1
        # ===== JARVIS PATCH END =====
        if classify_mode:
            # Taps leave through the return value, not through the caller's
            # tap_outputs list -- see model_sp.py's classify_mode branch for why
            # that list cannot cross an FSDP boundary. t_emb rides along because
            # the discriminator's AdaLN input is derived from it.
