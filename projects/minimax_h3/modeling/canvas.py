...
# ===== JARVIS PATCH BEGIN(sglang runtime coupling: task_profiles is serving-side request validation, not vendored; only used by the commented-out plan validators below) =====
...
# ===== JARVIS PATCH BEGIN(sglang runtime coupling: these consume sglang serving `batch`/`plan` objects and the prequeue/material_io stages, none of which are vendored; the pure canvas geometry above is kept) =====
...
__all__ = [
    ...
    # ===== JARVIS PATCH BEGIN(follows the commented-out serving-coupled definition above) =====
